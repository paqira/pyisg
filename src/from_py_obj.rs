...
impl<'a> FromPyObject<'a> for HeaderWrapper {
    fn extract_bound(ob: &Bound<'a, PyAny>) -> PyResult<Self> {
        let obj = ob
            .downcast::<PyDict>()
            .map_err(|_| SerError::new_err("unexpected type on `header`, expected dict"))?;

        let model_name = obj
            .get_item("model_name")?
            .map_or(Ok(None), |obj| obj.extract())
            .map_err(|_| {
                SerError::new_err("unexpected type on `model_name`, expected str | None")
            })?;
        let model_year = obj
            .get_item("model_year")?
            .map_or(Ok(None), |obj| obj.extract())
            .map_err(|_| {
                SerError::new_err("unexpected type on `model_year`, expected str | `None`")
            })?;
        let model_type = obj
            .get_item("model_type")?
        ...
        let data_format = obj
            ...
            .map_err(|_| {
                SerError::new_err("unexpected type on `data_format`, expected 'grid' | 'sparse'")
            })?
            .into();
        let data_ordering = obj
            ...
            .transpose()
            .map_err(|_| SerError::new_err("unexpected type on `data_ordering`, expected 'N-to-S, W-to-E' | 'lat, lon, N' | 'east, north, N' | 'N' | 'zeta' | None"))?
            .map(Into::into);
        let ref_ellipsoid = obj
            .get_item("ref_ellipsoid")?
            .map_or(Ok(None), |obj| obj.extract())
            .map_err(|_| {
                SerError::new_err("unexpected type on `ref_ellipsoid`, expected str | None")
            })?;
        let ref_frame = obj
            .get_item("ref_frame")?
            .map_or(Ok(None), |obj| obj.extract())
            .map_err(|_| {
                SerError::new_err("unexpected type on `ref_frame`, expected str | None")
            })?;
        let height_datum = obj
            .get_item("height_datum")?
            .map_or(Ok(None), |obj| obj.extract())
            .map_err(|_| {
                SerError::new_err("unexpected type on `height_datum`, expected str | None")
            })?;
        let tide_system = obj
            .get_item("tide_system")?
            .map_or(Ok(None), |obj| obj.extract::<Option<TideSystemWrapper>>())
            .map_err(|_| SerError::new_err("unexpected type on `tide_system`, expected 'tide-free' | 'mean-tide' | 'zero-tide' | None"))?
            .map(Into::into);
        let coord_type = obj
            .get_item("coord_type")?
        ...
        let map_projection = obj
            ...
            .map_or(Ok(None), |obj| obj.extract())
            .map_err(|_| {
                SerError::new_err("unexpected type on `map_projection`, expected str | None")
            })?;
        ...
        let EPSG_code = obj
            ...
            .map_or(Ok(None), |obj| obj.extract())
            .map_err(|_| {
                SerError::new_err("unexpected type on `EPSG_code`, expected str | None")
            })?;
        let nrows = obj
            .get_item("nrows")?
            .ok_or(SerError::new_err("missing `nrows`"))?
            .extract()
            .map_err(|_| SerError::new_err("unexpected type on `nrows`, expected int (usize)"))?;
        let ncols = obj
            .get_item("ncols")?
            .ok_or(SerError::new_err("missing `ncols`"))?
            .extract()
            .map_err(|_| SerError::new_err("unexpected type on `ncols`, expected int (usize)"))?;
        let nodata = obj
            .get_item("nodata")?
            .map_or(Ok(None), |obj| obj.extract())
        ...
        let ISG_format = obj
            ...
            .extract()
            .map_err(|_| {
                SerError::new_err("unexpected type on `ISG_format`, expected str | None")
            })?;
        ...
        let data_bounds = match coord_type {
            CoordType::Geodetic => {
                let lat_min = obj
                    ...
                    .map_err(|_| SerError::new_err("unexpected type on `lat_min`, expected float | { degree: int (i16), minutes: int (u8), second: int (u8) }"))?
                    .into();
                let lat_max = obj
                    .get_item("lat_max")?
                    .ok_or(SerError::new_err("missing `lat_max`"))
                    .map_err(|_| SerError::new_err("unexpected type on `lat_max`, expected float | { degree: int (i16), minutes: int (u8), second: int (u8) }"))?
                    .extract::<CoordWrapper>()?
                    .into();
                let lon_min = obj
                    .get_item("lon_min")?
                    .ok_or(SerError::new_err("missing `lon_min`"))
                    .map_err(|_| SerError::new_err("unexpected type on `lon_min`, expected float | { degree: int (i16), minutes: int (u8), second: int (u8) }"))?
                    .extract::<CoordWrapper>()?
                    .into();
                let lon_max = obj
                    .get_item("lon_max")?
                    .ok_or(SerError::new_err("missing `lon_max`"))
                    .map_err(|_| SerError::new_err("unexpected type on `lon_max`, expected float | { degree: int (i16), minutes: int (u8), second: int (u8) }"))?
                    .extract::<CoordWrapper>()?
                    .into();
                ...
                match data_format {
                    DataFormat::Grid => {
                        let delta_lat = obj
                            ...
                            .extract::<CoordWrapper>()
                            .map_err(|_| SerError::new_err("unexpected type on `delta_lat`, expected float | { degree: int (i16), minutes: int (u8), second: int (u8) }"))?
                            .into();
                        let delta_lon = obj
                            .get_item("delta_lon")?
                            .ok_or(SerError::new_err("missing `delta_lon`"))?
                            .extract::<CoordWrapper>()
                            .map_err(|_| SerError::new_err("unexpected type on `delta_lon`, expected float | { degree: int (i16), minutes: int (u8), second: int (u8) }"))?
                            .into();
            ...
            CoordType::Projected => {
                let north_min = obj
                    ...
                    .extract::<CoordWrapper>()
                    .map_err(|_| SerError::new_err("unexpected type on `north_min`, expected float | { degree: int (i16), minutes: int (u8), second: int (u8) }"))?
                    .into();
                let north_max = obj
                    .get_item("north_max")?
                    .ok_or(SerError::new_err("missing `north_max`"))?
                    .extract::<CoordWrapper>()
                    .map_err(|_| SerError::new_err("unexpected type on `north_max`, expected float | { degree: int (i16), minutes: int (u8), second: int (u8) }"))?
                    .into();
                let east_min = obj
                    .get_item("east_min")?
                    .ok_or(SerError::new_err("missing `east_min`"))?
                    .extract::<CoordWrapper>()
                    .map_err(|_| SerError::new_err("unexpected type on `east_min`, expected float | { degree: int (i16), minutes: int (u8), second: int (u8) }"))?
                    .into();
                let east_max = obj
                    .get_item("east_max")?
                    .ok_or(SerError::new_err("missing `east_max`"))?
                    .extract::<CoordWrapper>()
                    .map_err(|_| SerError::new_err("unexpected type on `east_max`, expected float | { degree: int (i16), minutes: int (u8), second: int (u8) }"))?
                    .into();
                ...
                match data_format {
                    DataFormat::Grid => {
                        let delta_north = obj
                            ...
                            .extract::<CoordWrapper>()
                            .map_err(|_| SerError::new_err("unexpected type on `delta_north`, expected float | { degree: int (i16), minutes: int (u8), second: int (u8) }"))?
                            .into();
                        let delta_east = obj
                            .get_item("delta_east")?
                            .ok_or(SerError::new_err("missing `delta_east`"))?
                            .extract::<CoordWrapper>()
                            .map_err(|_| SerError::new_err("unexpected type on `delta_east`, expected float | { degree: int (i16), minutes: int (u8), second: int (u8) }"))?
                            .into();
...
impl<'a> FromPyObject<'a> for DataWrapper {
    fn extract_bound(ob: &Bound<'a, PyAny>) -> PyResult<Self> {
        if let Ok(data) = ob.extract() {
            ...
            Err(SerError::new_err("unexpected type on `data`, expected list[list[float | None]] | list[tuple[float | { degree: int (i16), minutes: int (u8), second: int (u8) }, float | { degree: int (i16), minutes: int (u8), second: int (u8) }, float]]"))
        }
    }
}
